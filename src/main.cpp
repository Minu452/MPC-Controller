    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // position is zero as we transformed into the vechicle frame
          double cte = polyeval(coeffs, 0);
          // As position is zero, higher order coefficents become zero
          double epsi = -atan(coeffs[1]);

          Eigen::VectorXd current_state(6);
          // state vector
          // x, y, psi, velocity, cross_track_error, error_psi
